...
Wraps the ``open-wallet-standard`` FFI module to implement the Pay
...
OWS_WALLET_ID is set AND ``open-wallet-standard`` is installed. If the
env var is set but the package is missing, creation fails loud with
install instructions.

Usage::

    signer = OwsSigner(wallet_id="pay-my-agent")
...
from payskill.signer import Signer
...
class OwsSigner(Signer):
    ...
    async def sign_typed_data(
        ...
        # 2. Build EIP712Domain type array dynamically from domain fields.
    ...
    def sign(self, hash_bytes: bytes) -> bytes:
        ...
        raise NotImplementedError(
            "OwsSigner does not support sign(). "
